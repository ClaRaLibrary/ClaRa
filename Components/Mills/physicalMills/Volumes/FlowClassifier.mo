...
model FlowClassifier "Aerosol component | flow classifier | for replaceable flow classifying processes | discretised fuel mass balance"
...
//__________________________________________________________________________//
  ...
  //-------------------------------------------------------------------------------
  //replaceable models
  replaceable model Classifying =  Fundamentals.Classifying.Classifying_flow constrainedby Fundamentals.Classifying.Classifying_flow_base                                                                          annotation(Dialog(group="Replaceable Models"),choicesAllMatching=true);
  Classifying classifying annotation (Placement(transformation(extent={{20,80},{40,100}})));

  replaceable model Geometry = ClaRa.Basics.ControlVolumes.Fundamentals.Geometry.GenericGeometry(volume=volume,z_in=z_in,z_out=z_out,A_cross=A_cross) constrainedby ClaRa.Basics.ControlVolumes.Fundamentals.Geometry.GenericGeometry "1st: choose geometry definition | 2nd: edit corresponding record" annotation (Dialog(group="Geometry"), choicesAllMatching=true);
  ...
  //-------------------------------------------------------------------------------
  //iCom
  inner Fundamentals.Records.iCom_FlowClassifier iComClassifier(
    n=n,
    ...
    A_cross=A_cross,
    p_gas_in=gasInlet.p,
    p_gas_out=gasOutlet.p) annotation (Placement(transformation(extent={{20,56},{40,76}})));
  ...
  model Outline
    ...
    input ClaRa.Basics.Units.Length height "length of transport volume";
  end Outline;
  ...
  Summary summary(
    outline(mass_fuel=mass_fuel, height=height),
    gasPortIn(
      mediumModel=gas,
    ...
    gasPortOut(
      ...
      h=gasOut.h,
      xi=gasOut.xi,
      H_flow=gasOut.h*gasOutlet.m_flow)) annotation (Placement(transformation(extent={{80,56},{100,76}})));
...
public
  parameter ClaRa.Basics.Units.Length height = 4.6 "length of transport volume" annotation(Dialog(tab="General",group="Geometry"));
  parameter ClaRa.Basics.Units.Area A_cross = 6.43 "cross section of transport volume" annotation(Dialog(tab="General",group="Geometry")); //1.1*2.86
  parameter Integer n = 10 "number of vertical discritization" annotation(Dialog(tab="General",group="Geometry"));

protected
  parameter ClaRa.Basics.Units.Length z_in[1] = {0};
  parameter ClaRa.Basics.Units.Length z_out[1] = {height};
  parameter ClaRa.Basics.Units.Length delta_height = height/n "length of transport volume";
  parameter ClaRa.Basics.Units.Volume volume = A_cross*height "transport volume";
  parameter ClaRa.Basics.Units.Volume delta_volume = volume/n "transport volume";
...
public
  ...
  Real classFraction_discrete[n,classification.N_class] "classFraction of each ring element";
  ...
  ClaRa.Basics.Units.MassFlowRate m_flow_down[n,classification.N_class];
  ClaRa.Basics.Units.MassFlowRate m_flow_up[n,classification.N_class];
  ClaRa.Basics.Units.Velocity w_prtcl_abs_down[n,classification.N_class]
                                                                        "";
...
//inlet mass flow for first element
  ...
  // DISCRETE MASS BALANCES ----------------------------------------------------------------------------
  der(mass_fuel_discrete[1,:]) = - m_flow_up[1,:] - m_flow_down[1,:] + m_flow_in[1,:] + m_flow_down[2,:];
  for i in 2:n-1 loop
    der(mass_fuel_discrete[i,:]) = - m_flow_up[i,:] - m_flow_down[i,:] + m_flow_up[i-1,:] + m_flow_down[i+1,:];
  end for;
  der(mass_fuel_discrete[n,:]) = - m_flow_up[n,:] - m_flow_down[n,:] + m_flow_up[n-1,:];

  //separation condition particle size for each particle size and associated particle mass fraction
  for i in 1:n loop
    for j in 1:classification.N_class loop

      m_flow_up[i,j] = max(classifying.w_prtcl_abs[i,j],0) .* rho_prtcl .* A_cross .* classifying.K_v[i,j];
      m_flow_down[i,j] = w_prtcl_abs_down[i,j] .* rho_prtcl .* A_cross .* classifying.K_v[i,j];
      w_prtcl_abs_down[i,j] = max(0.1*classifying.w_gas-max(classifying.w_prtcl_abs[i,j],0),-1*min(classifying.w_prtcl_abs[i,j],0));

    end for;

    //classFration for each element
    classFraction_discrete[i,:] = mass_fuel_discrete[i,:]./sum(mass_fuel_discrete[i,:]);
  end for;

  //outlet mass flow to classifier -------------------------------------------------------------------
  fuelOutlet1.m_flow = - sum(m_flow_up[n,:]);  // mass flow to classifier
  fuelOutlet1.classFraction_outflow = m_flow_up[n,1:classification.N_class-1] ./ max(Modelica.Constants.small,sum(m_flow_up[n,:]));

  //return outlet mass flow to table -------------------------------------------------------------------
  fuelOutlet2.m_flow = - sum(m_flow_down[1,:]);
  fuelOutlet2.classFraction_outflow = m_flow_down[1,1:classification.N_class-1] ./ max(Modelica.Constants.small,sum(m_flow_down[1,:]));

  // total mass in controll volume
...
<p>For revisions please consult the html-documentation shipped with ClaRa.</p>
    ...
    Icon(coordinateSystem(preserveAspectRatio=false), graphics={Bitmap(
          ...
          fileName="modelica://ClaRa/Resources/Images/Components/VolumeDistr.png")}),
