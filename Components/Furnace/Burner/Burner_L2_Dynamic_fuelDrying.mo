...
protected
     TILMedia.Gas_pT    primaryAir_inlet(p=fuelFlueGas_inlet.flueGas.p, T=noEvent(actualStream(fuelFlueGas_inlet.flueGas.T_outflow)), xi=noEvent(actualStream(
...
equation
  ...
  + outlet.slag.m_flow*(outlet.slagType.cp*(noEvent(actualStream(outlet.slag.T_outflow)) - T_0) - h_flueGas_out)
  + inlet.slag.m_flow*(inlet.slagType.cp*(noEvent(actualStream(inlet.slag.T_outflow)) - T_0) - h_flueGas_out)
...
// + m_flow_evap*(h_fuel_water_in - h_flueGas_out) - m_flow_evap*(h_fuel_water_out- h_flueGas_out)
  ...
  if noEvent((1-sum(elementaryComposition_fuel_in)) <= 1e-9 or (-outlet.fuel.m_flow <= 1e-9)) then
