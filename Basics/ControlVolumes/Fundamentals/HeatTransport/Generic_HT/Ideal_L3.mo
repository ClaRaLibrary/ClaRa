...
//__________________________________________________________________________//
  ...
  constant ClaRa.Basics.Units.CoefficientOfHeatTransfer alpha_nom[iCom.N_cv]=ones(iCom.N_cv)*Modelica.Constants.inf "Constant nominal heat transfer coefficient || [1]:= liq | [2]:= vap ";
  constant ClaRa.Basics.Units.ThermalResistance HR_nom[iCom.N_cv]=zeros(iCom.N_cv) "Nominal convective heat resistance";
...
equation
     heat.T = iCom.T;
     alpha = ones(iCom.N_cv)*Modelica.Constants.inf;
     HR = zeros(iCom.N_cv);
