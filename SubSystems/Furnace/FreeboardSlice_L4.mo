...
//__________________________________________________________________________//
  ...
  extends ClaRa.Basics.Icons.FreeboardSlice;
...
/// Furnace ///
  ...
  parameter TILMedia.Gas.Types.BaseGas flueGas=simCenter.flueGasModel "Flue gas model" annotation (choicesAllMatching,
      Dialog(group="Media Definitions", groupImage=
          "modelica://ClaRa/Resources/Images/ParameterDialog/FurnaceSketchEmpty.png"));
  ...
  parameter Boolean additionalFuelGasConnector= true "True if an additional fuel and gas connector is available" annotation (choicesAllMatching, Dialog(group="Fundamental Definitions"));
  ...
  replaceable model ParticleMigration = ClaRa.Components.Furnace.GeneralTransportPhenomena.ParticleMigration.FixedMigrationSpeed_simple (w_fixed=1)
...
// Finned Tube Walls (FTW)///
  ...
  ClaRa.Components.Furnace.FlameRoom.FlameRoomAdditionalAir_L2_Dynamic flameRoom(
    fuelModel=fuelModel,
    slagType=slagType,
    flueGas=flueGas,
  ...
  ClaRa.Components.VolumesValvesFittings.Pipes.PipeFlowVLE_L4_Simple pipeFlow_FTW(
    ...
    Delta_x=Delta_x_FTW)
        ...
        rotation=90,
        origin={280,0})));
  ClaRa.Basics.Interfaces.FuelFlueGas_inlet inletFG2(fuelModel=fuelModel, flueGas(Medium=flueGas)) if additionalFuelGasConnector
...
equation
  ...
  connect(pipeFlow_FTW.inlet, inlet_FTW) annotation (Line(
      ...
      pattern=LinePattern.Solid,
      thickness=0.5));
  connect(flameRoom.fuelFlueGas_inlet, inletFG2) annotation (Line(
      points={{-52,0},{-300,0}},
      color={118,106,98},
      thickness=0.5));
