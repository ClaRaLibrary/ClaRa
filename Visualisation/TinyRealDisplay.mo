...
//__________________________________________________________________________//
  ...
  parameter Real u_high = 1 "Red text for u>u_high";
  parameter Real u_low= 0 "Red text for u<u_low";
...
</html>"),Icon(coordinateSystem(preserveAspectRatio=false), graphics={Line(points={{-100,0},{100,0}}, color={0,0,127}), Text(
          extent={{-100,80},{100,20}},
          lineColor=DynamicSelect({0,0,127}, if u >= u_low and u <= u_high then {0,0,127} else {205, 79, 70}),
          textString=DynamicSelect(" x ", String(u,format = "1."+String(digits)+"f")))}),                                     Diagram(graphics,
