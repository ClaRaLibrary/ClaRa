...
//__________________________________________________________________________//
  ...
  parameter TILMedia.VLEFluid.Types.BaseVLEFluid vleMedium=simCenter.fluid1 "vleMedium in the component"
    annotation (Dialog(group="Fundamental Definitions"));
  parameter TILMedia.Gas.Types.BaseGas flueGas=simCenter.flueGasModel "Flue gas model used in component"
    annotation (Dialog(group="Fundamental Definitions"));

  parameter ClaRa.Basics.Units.EnthalpyMassSpecific h_vle_bundle_out_nom "Nominal vle bundle outlet spec. enthalpy" annotation (Dialog(group="Nominal Operation Point"));
  ...
  parameter Real CharLine_h_bundle_P_target_[:,2]=[0,1;1,1] "Characteristic line of h_vle_bundle_out as function of P_target_" annotation(Dialog(group="Part Load Definition"));
...
// _________GENERAL VALUES ___________
  ...
  final parameter ClaRa.Basics.Units.EnthalpyMassSpecific h_bub_bundle=TILMedia.VLEFluid.MixtureCompatible.Functions.bubbleSpecificEnthalpy_pxi(                                     vleMedium, p_vle_bundle_out) "Rprt: Bubble enthalpy at vle outlet";
  final parameter ClaRa.Basics.Units.EnthalpyMassSpecific h_dew_bundle=TILMedia.VLEFluid.MixtureCompatible.Functions.dewSpecificEnthalpy_pxi(                                     vleMedium, p_vle_bundle_out) "Rprt: Dew enthalpy at vle outlet";

  final parameter ClaRa.Basics.Units.EnthalpyMassSpecific h_bub_wall=TILMedia.VLEFluid.MixtureCompatible.Functions.bubbleSpecificEnthalpy_pxi(                                     vleMedium, p_vle_wall_out);
  final parameter ClaRa.Basics.Units.EnthalpyMassSpecific h_dew_wall=TILMedia.VLEFluid.MixtureCompatible.Functions.dewSpecificEnthalpy_pxi(                                     vleMedium, p_vle_wall_out);

  final parameter ClaRa.Basics.Units.MassFlowRate m_flow_vle_bundle(fixed=false) "Mass flow rate vle medium";
  final parameter ClaRa.Basics.Units.Pressure Delta_p_vle_bundle(fixed=false) "Pressure losses vle medium";
  ...
  final parameter ClaRa.Basics.Units.Pressure Delta_p_geo_wall=TILMedia.VLEFluid.MixtureCompatible.Functions.density_phxi(
      vleMedium,
      p_vle_wall_out,
      h_vle_wall_out,
      xi)*Modelica.Constants.g_n*(z_wall_out - z_wall_in) "Geostatic pressure difference";

  final parameter ClaRa.Basics.Units.Pressure Delta_p_geo_bundle=TILMedia.VLEFluid.MixtureCompatible.Functions.density_phxi(
      vleMedium,
      p_vle_bundle_out,
      h_vle_bundle_out,
  ...
  final parameter ClaRa.Basics.Units.Temperature T_vle_bundle_in=TILMedia.VLEFluid.MixtureCompatible.Functions.temperature_phxi(
      vleMedium,
      p_vle_bundle_in,
...
// _________VLE Outlet ________________
  final parameter ClaRa.Basics.Units.EnthalpyMassSpecific h_vle_bundle_out(fixed=false) "Outlet specific enthalpy";
  final parameter ClaRa.Basics.Units.Temperature T_vle_bundle_out=TILMedia.VLEFluid.MixtureCompatible.Functions.temperature_phxi(
      vleMedium,
      p_vle_bundle_out,
      h_vle_bundle_out) "VLE  medium's outlet temperature";
  final parameter ClaRa.Basics.Units.Pressure p_vle_bundle_out(fixed=false) "VLE  medium's outlet pressure";
...
// _________GAS Inlet _________________
  final parameter ClaRa.Basics.Units.EnthalpyMassSpecific h_fg_in=(m_flow_fg*h_fg_out - m_flow_vle_bundle_in*h_vle_bundle_in + m_flow_vle_bundle*h_vle_bundle_out - m_flow_vle_wall_in*h_vle_wall_in + m_flow_vle_wall_in*h_vle_wall_out)/m_flow_fg "FG medium's inlet specific enthalpy flue gas";
...
protected
  ...
  Modelica.Blocks.Tables.CombiTable1Dv table2(table=CharLine_h_bundle_P_target_, u={P_target_});
...
public
  ClaRa.StaticCycles.Fundamentals.SteamSignal_blue_a inletBundle(p=p_vle_bundle_in, Medium=vleMedium) annotation (Placement(transformation(
        ...
        rotation=0,
        origin={104,20})));
  ClaRa.StaticCycles.Fundamentals.SteamSignal_blue_b outletBundle(h=h_vle_bundle_out, m_flow=m_flow_vle_bundle, Medium=vleMedium) annotation (Placement(transformation(
        extent={{10,-10},{-10,10}},
        rotation=270,
        origin={100,-30}), iconTransformation(
        extent={{4,-10},{-4,10}},
...
initial equation
  ...
  outletBundle.p = p_vle_bundle_out;
...
</html>"),Dialog(group="Discretisation (for reporting only)"), choices(checkBox=false),
              Diagram(graphics,
